...
class ubuntu_performance_lkp(test.test):
    ...
    def setup(self, lkp_jobs, lkp_commit):
        ...
        for lkp_job in lkp_jobs:
            ...
            utils.system_output('yes "" | lkp install %s || true' % lkp_jobs[lkp_job][0], retain_output=True)
    ...
    def run_once(self, lkp_job, sub_job, lkp_jobs, lkp_commit = None):
        ...
        job_funcs = {
            ...
            'hackbench-100.yaml':    self.run_hackbench,
            'hackbench-8.yaml':      self.run_hackbench,
            'hackbench-perf.yaml':   self.run_hackbench,
