...
class ubuntu_xfstests_xfs(test.test):
    ...
    def _run_suite(self):
        os.chdir(os.path.join(self.srcdir, 'xfstests-bld', 'fstests-bld', 'xfstests-dev'))
        # Unlike btrfs and ext4, thers is no exclusion file for xfs
        utils.system_output('./check -g auto -x dangerous',
                            ignore_status=True,
                            retain_output=True)
    ...
    def run_once(self, test_name, filesystem='xfs', test_number='000', single=False, skip_dangerous=True):
        if test_name == 'setup':
            return
